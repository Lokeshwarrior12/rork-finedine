...
const ScratchCard = ({ 
  ...
  return (
    <View style={scratchStyles.scratchContainer}>
      <View style={scratchStyles.hiddenContent}>
        {children}
      </View>
      ...
      {!isRevealed && (
        ...
        >
          <View style={scratchStyles.scratchPattern}>
             {dimensions.width > 0 && Array.from({ length: totalBlocks }).map((_, i) => {
          ...
          <View style={styles.scratchWrapper}>
             <ScratchCard isRevealed={isRevealed} onReveal={handleReveal} colors={colors}>
                <View style={styles.codeBox}>
                  {showQr ? (
                    <Image 
                      source={{ uri: `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${coupon.code}` }}
                      style={styles.qrCode}
                      contentFit="contain"
                    />
                  ) : (
...
export default function CouponsScreen() {
  ...
  const filteredCoupons = userCoupons.filter(c => c.status === activeTab);

  const tabs: { key: TabType; label: string; count: number }[] = [
    { key: 'active', label: 'Active', count: userCoupons.filter(c => c.status === 'active').length },
    { key: 'used', label: 'Used', count: userCoupons.filter(c => c.status === 'used').length },
    { key: 'expired', label: 'Expired', count: userCoupons.filter(c => c.status === 'expired').length },
  ];
...
const createScratchStyles = (colors: any) => StyleSheet.create({
  scratchContainer: {
    position: 'relative',
    minHeight: 60,
  },
